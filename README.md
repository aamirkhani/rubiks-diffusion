...
A ~1-minute animated explainer (concept + the trained denoiser solving a real
100-move scramble in 3D): [`video/rubiks_diffusion.mp4`](video/rubiks_diffusion.mp4).
Fully generated by [`video/make_video.py`](video/make_video.py) from real engine
states and the trained model — no manual animation.
...
[`paper2/when-is-rl-a-diffusion-problem_r5.pdf`](paper2/when-is-rl-a-diffusion-problem_r5.pdf)
(earlier versions preserved: `scramble-inversion-beyond-groups.pdf` = r1,
`_r2`, and `when-is-rl-a-diffusion-problem_r3/_r4.pdf`)
...
actual trajectory back):

![Gallery A: domains 1-5](paper2/png/fig2_gallery_A.png)
![Gallery B: domains 6-10](paper2/png/fig2_gallery_B.png)

**One architecture for everything:**

![Architecture](paper2/png/fig2_architecture.png)

| Verdict | Domains | Why |
|---|---|---|
| ✅ works (often dominant) | 24-puzzle, mazes, POMDP maze, Hanoi, Lights Out, Mountain Car | reversible + deterministic; scale favors the denoiser |
| ⚠️ works with a fix | Sokoban & Peg Solitaire (add value veto), pendulum (distributional head) | deadlock-blindness; mean-collapse |
| ❌ structural failure | 2048 | stochastic dynamics: the reverse process models a game never played |
