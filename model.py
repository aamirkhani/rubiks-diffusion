...
class PolicyNet(nn.Module):
    ...
    def __init__(self, S, M, h1=5000, h2=1000, blocks=4, t_dim=0):
        super().__init__()
        self.S, self.M, self.t_dim = S, M, t_dim
        self.fc1 = nn.Linear(S * 6 + t_dim, h1)
    ...
    def forward(self, states, t=None):
        x = F.one_hot(states.long(), 6).to(torch.float32).flatten(1)
        if self.t_dim > 0:
...
class ValueNet(nn.Module):
    ...
    def __init__(self, S, h1=5000, h2=1000, blocks=4):
        super().__init__()
        self.S = S
        self.fc1 = nn.Linear(S * 6, h1)
        self.n1 = nn.LayerNorm(h1)
        self.fc2 = nn.Linear(h1, h2)
        self.n2 = nn.LayerNorm(h2)
        self.blocks = nn.ModuleList([ResBlock(h2) for _ in range(blocks)])
